...
public class tipoventabd {
    //Inserta un nuevo Registro
    public static void insertar(Tipoventa per) throws SQLException {
        Connection cnn = conectar.getConexion();//abrir una conexion
       PreparedStatement ps = null;
        
        String sql;

        sql="INSERT INTO tipoventa (tipo)VALUES (?)";
        try {
            ps=cnn.prepareStatement(sql);//preparar el codigo para devolver resgistros
            ps.setString(1,per.getTipos());//modelo de la clase va con mayus
            ...
            Logger.getLogger(frmtipoventa.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
