...
public class productobd {
    ...
    public static void insertar(Producto per) throws SQLException {
        Connection cnn = conectar.getConexion();//abrir una conexion
       PreparedStatement ps = null;
        
        String sql;

        sql="INSERT INTO producto (producto,modelo,caracteristicas,existente,precioCompra,precioVenta,estado,categoria_pk_categoria)VALUES (?,?,?,?,?,?,?,?)";
        try {   
            ps=cnn.prepareStatement(sql);//preparar el codigo para devolver resgistros
            ps.setString(1,per.getProducto());//modelo de la clase va con mayus
            ps.setString(2,per.getModelo());
            ps.setString(3,per.getCaracteristicas());
            ps.setString(4,per.getExistencia());
            ps.setString(5,per.getPreciocompra());
            ps.setString(6,per.getPrecioventa());
            ps.setString(7,per.getEstado());
             ps.setString(8,per.getIdcategoria_fk());
